...
from .attacktool import get_entries
...
from guided_diffusion.attacktool import VGGFeatureExtractor, gram_matrix
class A_DDIMSampler(DDIMSampler):
    def __init__(self, use_timesteps, conf=None, **kwargs):
        ...
        self.mode = conf.get("mode", "perturbation_attack")
        self.scale = conf.get("scale", 0)
        self.use_nearest_label = conf.get("use_nearest_label", True)
        self.coef_recon_reg = conf.get("coef_recon_reg", 1.0)
        self.coef_adv_reg = conf.get("coef_adv_reg", 0.0)
        self.target_model_name = conf.get("target_model","Pure_Resnet50")
        ...
        if self.mode == 'style':
            ...
            self.feature_extractor = VGGFeatureExtractor(content_layers_default, style_layers_default).to(
                self.device)
            self.coef_style_reg = conf.get("coef_style_reg", 1e5)
            self.coef_content_reg = self.coef_recon_reg
    ...
    def p_sample(
        ...
        if self.mode == "perturbation_attack":
            def loss_fn(_x0, _pred_x0, _mask, const, target_model, target_label,_x):
                if self.regional==True:
                    _pred_x0 = mask * x0 + (1 - mask) * _pred_x0
                i,j = get_entries(_pred_x0, target_model,target_label)
                if self.regional==True:
                    ret = self.coef_adv_reg*torch.sum((const-(j-i))**2)+self.coef_recon_reg*torch.sum((_x0*(1-_mask)-_pred_x0*(1-_mask))**2)
                else:
            ...
            def loss_fn(_x0, _pred_x0, _mask, const, target_model, target_label, _x):
                ...
                new_x0 = model_kwargs['reference_image']
                new_x0_norm = (new_x0 + 1) / 2
                pred_x0_norm = (_pred_x0 + 1) / 2
                if self.regional==True:
                    new_x0_norm = (1-mask)*new_x0_norm
                    pred_x0_norm = (1-mask)*pred_x0_norm

                _, new_x0_style_features = self.feature_extractor(new_x0_norm)
                _, pred_x0_style_features = self.feature_extractor(pred_x0_norm)

                style_layer_weights = [1.0 / len(pred_x0_style_features)] * len(pred_x0_style_features)
                style_constraint = 0
                for weight, f1, f2 in zip(style_layer_weights, pred_x0_style_features, new_x0_style_features):
                    gram1 = gram_matrix(f1)
                    gram2 = gram_matrix(f2)
                    style_constraint += weight * torch.mean((gram1 - gram2) ** 2)
                ret = self.coef_adv_reg * torch.sum((const - (j - i)) ** 2) + self.coef_style_reg * style_constraint
                return ret

        elif self.mode == "colorization" :
            def loss_fn(_x0, _pred_x0, _mask, const, target_model, target_label, _x):
                _ref_x0  = model_kwargs['reference_image']

                x0_01 = (_x0 + 1.0) / 2.0
                pred_x0_01 = (_pred_x0 + 1.0) / 2.0
                ref_x0_01 = (_ref_x0 + 1.0) / 2.0

                x0_lab = Color.rgb_to_lab(x0_01)
                pred_x0_lab = Color.rgb_to_lab(pred_x0_01)
                ref_x0_lab = Color.rgb_to_lab(ref_x0_01)

                adv_target_lab = torch.cat(
                    [x0_lab[:, 0:1, :, :], pred_x0_lab[:, 1:3, :, :]], dim=1
                )
                adv_target_rgb_01 = Color.lab_to_rgb(adv_target_lab)
                adv_target_rgb = adv_target_rgb_01 * 2.0 - 1.0

                if self.regional==True:
                    _pred_x0 = mask * x0 + (1 - mask) *  adv_target_rgb

                i,j = get_entries(_pred_x0, target_model,target_label)

                if self.regional==True:
                    ret = self.coef_adv_reg*torch.sum((const-(j-i))**2)+self.coef_recon_reg*torch.sum((pred_x0_lab[:, 1:3, :, :]-ref_x0_lab[:, 1:3, :, :]*(1-_mask))**2)
                else:
                    ret = self.coef_adv_reg * torch.sum((const - (j - i)) ** 2) + self.coef_recon_reg * torch.sum(
                        ( pred_x0_lab[:, 1:3, :, :] - ref_x0_lab[:, 1:3, :, :]) ** 2)
                return ret
        else:
        ...
        def get_predx0(_x, _t, _et, interval_num=1):
            if interval_num == 1:
                pred_x0  = process_xstart(self._predict_xstart_from_eps(_x, _t, _et))
                return pred_x0
            else:
                _pred_x0 = grad_ckpt(
        ...
        with torch.enable_grad():
            ...
            for step in range(self.num_iteration_optimize_xt):
                ...
                if self.mode == 'colorization':
                    x0_01 = (x0 + 1.0) / 2.0
                    new_x_01 = (new_x + 1.0) / 2.0

                    x0_lab = Color.rgb_to_lab(x0_01)
                    new_x_lab = Color.rgb_to_lab(new_x_01)

                    combined_lab = torch.cat(
                        [x0_lab[:, 0:1, :, :], new_x_lab[:, 1:3, :, :]], dim=1
                    )
                    color_only_pred_x0_01 = Color.lab_to_rgb(combined_lab)
                    new_x = color_only_pred_x0_01 * 2.0 - 1.0
        ...
        # after optimize
        with torch.no_grad():
            new_loss = loss_fn(x0, pred_x0, mask, const, target_model, target_label, x).item()
    ...
    def p_sample_loop(
        ...
        origin_x = model_kwargs['gt']
        mask = model_kwargs['gt_keep_mask']
        logging_info(
            "the adaptive t_star is: %d, the adaptive const is: %.5lf"
            % (t_star, adaptive_const)
        )
        lr_xt = self.lr_xt
        ...
        for i, [cur_t, prev_t] in tqdm(enumerate(time_pairs)):
            if cur_t > time:
                continue
